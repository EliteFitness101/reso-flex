...
export default function OrderStatus() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      try {
        ...
        if (payload.status === "paid" && !escalated.current) {
          escalated.current = true;
          const sku = payload.items?.[0]?.sku || "";
          const message = `Hi ResoFlex — I have successfully completed payment.\n\nReference: ${payload.reference}\nSKU: ${sku || "order"}\nAmount: ${ngn(payload.amount)}\n\nPlease confirm my order and proceed with fulfillment.`;
          const url = waUrl({ override: message, source: `paid_${payload.reference}` });
          window.setTimeout(() => { window.location.assign(url); }, 1200);
          return;
        }
        if (payload && TERMINAL.has(payload.status)) return;
  ...
  const statusCopy = (s?: string) => {
    if (s === "paid") return { title: "Payment verified — connecting you to ResoFlex WhatsApp…", tone: "text-emerald-400" };
    ...
    return { title: "Payment received. We're verifying your transaction.", tone: "text-gold" };
  };
  ...
  const continueWhatsApp = () => {
    ...
    window.location.assign(waUrl({ override: message, source: `paid_${order?.reference || reference}` }));
  };
        ...
        <div className="mt-6 border border-gold/40 bg-noir-900/70 p-7 shadow-elevated">
          ...
          {order && (
            <div className="mt-6 space-y-4 text-sm">
              ...
              {order.status === "paid" && <button onClick={continueWhatsApp} className="luxury-button w-full py-3 text-[11px]"><i className="fa-brands fa-whatsapp mr-2" /> Continue to WhatsApp Fulfillment</button>}
              ...
              {order.items && order.items.length > 0 && <div className="border-t border-border/40 pt-4"><div className="text-[10px] uppercase tracking-[0.25em] text-foreground/50">Items</div><ul className="mt-2 space-y-1">{order.items.map((it, i) => <li key={i}>{it.sku || "item"} — {ngn(it.amount)}</li>)}</ul></div>}
              {order.download_links && order.download_links.length > 0 && <div className="border-t border-border/40 pt-4"><div className="text-[10px] uppercase tracking-[0.25em] text-foreground/50">Downloads</div><ul className="mt-2 space-y-1">{order.download_links.map((d, i) => <li key={i}><a href={d.url} className="text-gold underline" target="_blank" rel="noreferrer">{d.label}</a></li>)}</ul></div>}
              {order.coach_contact && <Row label="Coach" value={order.coach_contact} />}
              {order.next_steps && <div className="border-t border-border/40 pt-4 text-sm text-foreground/80">{order.next_steps}</div>}
            </div>
          )}
          {!order && !err && <div className="mt-6 text-xs text-foreground/50">Loading…</div>}
          {err && !order && <div className="mt-6 text-xs text-red-400">Network hiccup — retrying…</div>}
          {order && !TERMINAL.has(order.status) && <div className="mt-6 text-[10px] uppercase tracking-[0.25em] text-foreground/45">Auto-refreshing until payment is confirmed…</div>}
        </div>
      </div>
    </main>
  );
...
function ngn(koboUnknown: number | undefined) {
  return typeof koboUnknown === "number" ? new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN" }).format(koboUnknown / 100) : "—";
}

function Row({ label, value, mono }: { label: string; value: React.ReactNode; mono?: boolean }) {
  return <div className="flex items-baseline justify-between gap-4 border-b border-border/30 pb-2"><span className="text-[10px] uppercase tracking-[0.25em] text-foreground/55">{label}</span><span className={mono ? "font-mono text-sm" : "text-sm"}>{value}</span></div>;
}
