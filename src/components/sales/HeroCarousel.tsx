...
const HERO_VIDEO_PORTRAIT = "https://ab2ttlkn9no0tuoa.public.blob.vercel-storage.com/buffer/assets/hero/resoflex-10s-cinematic-hero-fitness-wellness-shopping.mp4.MP4";
const HERO_VIDEO_LANDSCAPE = "https://ab2ttlkn9no0tuoa.public.blob.vercel-storage.com/buffer/assets/hero/resoflex-landscape-10s-cinematic-hero-fitness-wellness-shopping.mp4.MP4";
const BG_HERO = "https://ab2ttlkn9no0tuoa.public.blob.vercel-storage.com/buffer/assets/hero/resoflex-10s-cinematic-hero-fitness-wellness-shopping.mp4.MP4";
const SLIDES = [
...
export const HeroCarousel = () => {
  ...
  useEffect(() => { const id = setInterval(() => { const t = videoRef.current?.currentTime ?? 0; setPhase(t < 3 ? 0 : t < 6 ? 1 : t < 8 ? 2 : 3); }, 250); return () => clearInterval(id); }, []);
  ...
  return <section id="top" className="relative overflow-hidden pt-4 pb-16 md:pt-10 md:pb-24" aria-label="ResoFlex shop hero">
    <div className="absolute inset-0 -z-10 bg-noir-950"><img src={BG_HERO} alt="" aria-hidden width={1080} height={1920} fetchPriority="high" className="absolute inset-0 h-full w-full object-cover opacity-25" /><div className="absolute inset-0 bg-gradient-to-b from-noir-950/45 via-noir-950/75 to-noir-950" /></div>
    <div className="container">
      ...
      <div className="relative mx-auto mt-7 aspect-[9/16] max-w-6xl overflow-hidden border border-gold/30 bg-noir-900 shadow-[var(--shadow-elevated)] sm:aspect-[16/8]">
        <video ref={videoRef} className="absolute inset-0 h-full w-full object-cover" autoPlay muted loop playsInline preload="metadata" poster={BG_HERO} aria-label="ResoFlex brand film">
          <source src={HERO_VIDEO_PORTRAIT} type="video/mp4" media="(max-width: 639px)" />
          <source src={HERO_VIDEO_LANDSCAPE} type="video/mp4" media="(min-width: 640px)" />
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 sm:p-8">
          ...
          {phase === 3 && <div className="max-w-xl"><div className="text-[10px] uppercase tracking-[0.3em] text-gold">8–10 sec · Start</div><div className="mt-1 font-display text-2xl font-bold sm:text-4xl">Your next upgrade is here.</div><div className="mt-4 flex flex-wrap gap-2"><a href="#products" onClick={() => cta("hero_start_shopping")} className="luxury-button rounded-full px-5 py-3 text-xs font-bold">Start shopping</a><span className="rounded-full border border-gold/30 bg-noir-900/70 px-5 py-3 text-xs font-bold text-gold">shop.resofit.fit</span></div></div>}
        </div>
      </div>
      <div className="mt-4 grid grid-cols-4 text-center text-[9px] uppercase tracking-[0.16em] text-foreground/45"><span className={phase === 0 ? "text-gold" : ""}>Discover</span><span className={phase === 1 ? "text-gold" : ""}>Shop</span><span className={phase === 2 ? "text-gold" : ""}>Refer & earn</span><span className={phase === 3 ? "text-gold" : ""}>Start</span></div>
