...
function getConfig(): CloudinaryConfig {
  ...
  if (value) {
    ...
    const fallback = { cloudName: parsed.hostname, apiKey: decodeURIComponent(parsed.username), apiSecret: decodeURIComponent(parsed.password) };
    if (fallback.cloudName && fallback.apiKey && fallback.apiSecret) return fallback;
  }
...
function normalizeFolder(value: unknown): string | null {
  ...
  if (!folder || (folder !== CANONICAL_ROOT && !folder.startsWith(`${CANONICAL_ROOT}/`))) return null;
  return folder;
}
...
async function search(config: CloudinaryConfig, expression: string, nextCursor?: string): Promise<SearchResponse> {
  ...
  const body = new URLSearchParams({ expression, max_results: String(MAX_RESULTS), timestamp: String(timestamp), api_key: config.apiKey, signature: sign({ expression, max_results: MAX_RESULTS, timestamp }, config.apiSecret), ...(nextCursor ? { next_cursor: nextCursor } : {}) });
  const response = await fetch(`${CLOUDINARY_API}/${encodeURIComponent(config.cloudName)}/resources/search`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body, signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.query.folder !== undefined && !folder) return res.status(400).json({ error: `folder must resolve under ${CANONICAL_ROOT}` });
    let assets: CloudinaryResource[];
    let mode: 'folder' | 'expression' | 'public_ids';
    if (folder) { assets = await collect(config, `asset_folder:"${folder}"`); mode = 'folder'; }
    else if (expression) { assets = await collect(config, expression); mode = 'expression'; }
    else if (publicIds.length) { assets = (await Promise.all(publicIds.map((id) => collect(config, `public_id:"${id}"`)))).flat(); mode = 'public_ids'; }
    ...
    return res.status(200).json({ status: 'PASS', cloudName: config.cloudName, root: CANONICAL_ROOT, mode, folder: folder ?? null, count: unique.length, assets: unique.map((asset) => ({ assetId: asset.asset_id, publicId: asset.public_id, assetFolder: asset.asset_folder ?? asset.folder, resourceType: asset.resource_type, type: asset.type, format: asset.format, width: asset.width, height: asset.height, bytes: asset.bytes, secureUrl: asset.secure_url })) });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    const status = /^Cloudinary search failed: (401|403)\b/.test(message) ? 502 : error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError') ? 504 : 500;
    console.error('[cloudinary/verify]', error);
    return res.status(status).json({ status: 'ERROR', error: 'Cloudinary verification failed', message });
  }
}
