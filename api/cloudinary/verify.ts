...
async function search(config: CloudinaryConfig, expression: string, nextCursor?: string): Promise<SearchResponse> {
  ...
  const response = await fetch(`${CLOUDINARY_API}/${encodeURIComponent(config.cloudName)}/resources/search`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body });
...
async function collectAll(config: CloudinaryConfig, expression: string) {
  ...
  do { const page = await search(config, expression, cursor); resources.push(...(page.resources ?? [])); cursor = page.next_cursor; } while (cursor);
  return resources;
}

function parsePublicIds(value: string | string[] | undefined) {
  if (!value) return [];
  return (Array.isArray(value) ? value : [value]).flatMap((item) => String(item).split(',')).map((id) => id.trim()).filter(Boolean);
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (folder) { assets = await collectAll(config, `asset_folder:"${folder.replace(/"/g, '\\"')}"`); mode = 'folder'; }
    else if (expression) { assets = await collectAll(config, expression); mode = 'expression'; }
    else if (publicIds.length) { assets = (await Promise.all(publicIds.map((id) => collectAll(config, `public_id:${id}`)))).flat(); mode = 'public_ids'; }
    else return res.status(400).json({ error: 'Provide folder, expression, or public_ids', usage: '/api/cloudinary/verify?folder=... | /api/cloudinary/verify?expression=... | /api/cloudinary/verify?public_ids=id1,id2,...' });
