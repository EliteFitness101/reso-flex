...
async function search(expression: string, nextCursor?: string): Promise<SearchResponse> {
  ...
  const body = new URLSearchParams({ expression, max_results: '500', timestamp: String(timestamp), api_key: API_KEY!, signature: sign(params), ...(nextCursor ? { next_cursor: nextCursor } : {}) });
  const response = await fetch(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/resources/search`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body });
...
async function collectAll(expression: string) {
  ...
  do { const page = await search(expression, cursor); all.push(...(page.resources ?? [])); cursor = page.next_cursor; } while (cursor);
  return all;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (publicIdsRaw) {
      ...
      assets = (await Promise.all(publicIds.map((publicId) => collectAll(`public_id:${publicId}`)))).flat();
      mode = 'public_ids';
    } else if (folderRaw) {
      assets = await collectAll(`asset_folder:"${folderRaw.replace(/"/g, '\\"')}"`);
      mode = 'asset_folder';
    } else if (expression) {
    ...
    return res.status(200).json({ ok: true, mode, count: unique.length, assets: unique });
  } catch (error) {
