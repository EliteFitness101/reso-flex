...
const fileNames = { hero: 'hero.png', gallery_01: 'gallery-01.png', gallery_02: 'gallery-02.png', gallery_03: 'gallery-03.png', lifestyle: 'lifestyle.png', detail: 'detail.png' } as const;

function blobUrl(folder: string, filename: string) {
  if (!BLOB_BASE) return null;
  return `${BLOB_BASE}/imagekit/assets/products/${folder}/${filename}`;
}
...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const imagekit = Object.fromEntries(Object.entries(IMAGEKIT_VERIFIED_MEDIA).map(([sku, media]) => [sku, {
    ...media,
    background: { image: blobUrl(media.folder, 'bg-hero.png'), video: blobUrl(media.folder, 'bg-hero.mp4') },
    assets: Object.fromEntries(Object.entries(media.assets).map(([role, asset]) => [role, asset ? {
      ...
      blobUrl: blobUrl(media.folder, fileNames[role as typeof roles[number]]),
    } : asset])),
  }]));
